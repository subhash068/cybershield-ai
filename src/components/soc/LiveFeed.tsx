    ...
    <ul className="divide-y divide-border">
      {items.map((e) => (
        <li key={e.id} className="px-1 py-2 flex items-center gap-3 text-xs">
          ...
          <span
            className={cn(
              "px-1.5 py-0.5 rounded border text-[10px] uppercase tracking-wider shrink-0",
          ...
          <span className="text-muted-foreground hidden lg:inline truncate flex-1">
            {e.user} → {e.target}
          </span>
          <span
            className={cn(
              ...
              e.status === "blocked" &&
                "text-[color:var(--neon-green)] bg-[color:var(--neon-green)]/10",
              e.status === "investigating" &&
                "text-[color:var(--neon-amber)] bg-[color:var(--neon-amber)]/10",
              e.status === "contained" &&
                "text-[color:var(--neon-cyan)] bg-[color:var(--neon-cyan)]/10",
              e.status === "allowed" &&
                "text-[color:var(--neon-red)] bg-[color:var(--neon-red)]/10 animate-blink",
            )}
          >
            {e.status}
          </span>
