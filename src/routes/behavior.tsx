...
function BehaviorPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader eyebrow="UEBA" title="User & Entity Behavior Analytics" description="Risk scoring, anomalous patterns and insider-threat detection across the workforce." />

      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        <Panel className="xl:col-span-2" title="Behavior Timeline" subtitle="Aggregate risk & anomaly count">
          <div className="h-[300px]">
            <ResponsiveContainer>
              <LineChart data={series}>
                ...
                <Line type="monotone" dataKey="risk" stroke="var(--neon-red)" strokeWidth={2} dot={false} />
                <Line type="monotone" dataKey="anomalies" stroke="var(--neon-cyan)" strokeWidth={2} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </Panel>
            ...
            {[
              ...
              <li key={l as string}>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="font-mono" style={{ color: c as string }}>{v}</span>
                </div>
                <div className="h-1.5 rounded-full bg-secondary overflow-hidden">
                  <div className="h-full" style={{ width: `${v}%`, background: c as string, boxShadow: `0 0 10px ${c}` }} />
                </div>
              </li>
            ))}
          </ul>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            ...
            <tbody className="divide-y divide-border">
              {userBehavior.sort((a, b) => b.risk - a.risk).map((u) => (
                  ...
                  <td className="py-2 px-2">
                    <div className="flex items-center gap-2">
                      <div className="w-28 h-2 rounded-full bg-secondary overflow-hidden">
                        <div className="h-full" style={{ width: `${u.risk}%`, background: riskColor(u.risk), boxShadow: `0 0 10px ${riskColor(u.risk)}` }} />
                      </div>
                      <span className="font-mono" style={{ color: riskColor(u.risk) }}>{u.risk}</span>
                    </div>
                  </td>
