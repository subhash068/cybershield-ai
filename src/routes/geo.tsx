...
function GeoPage() {
  return (
    <div className="space-y-6">
      <PageHeader eyebrow="Geolocation Intelligence" title="Global Cyber Attack Map" description="Origin countries, attack density, regional heatmaps and animated attack lines." />

      <div className="grid grid-cols-1 xl:grid-cols-4 gap-4">
        <Panel className="xl:col-span-3" title="Global Attack Visualization" padded={false}>
          <div className="h-[520px] p-3"><WorldMap events={attacks} max={90} /></div>
        </Panel>
                ...
                return (
                  <li key={c.code} className="text-xs">
                    <div className="flex items-center justify-between mb-1">
                      <span><span className="font-mono text-muted-foreground mr-2">{c.code}</span>{c.country}</span>
          ...
          <Panel title="Suspicious Regions">
            <ul className="space-y-2 text-xs">
              {[
                ...
                <li key={r.r} className="flex items-center justify-between rounded border border-border bg-card/40 px-2 py-1.5">
                  <span>{r.r}</span>
                  <span className="text-[10px] uppercase tracking-wider" style={{ color: r.c }}>{r.lvl}</span>
                </li>
              ))}
            </ul>
          </Panel>
