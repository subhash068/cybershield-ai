  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Active incidents, severity gauges, analyst assignment and response status timeline."
      />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          {
            l: "Open",
            v: incidents.filter((i) => i.status === "open").length,
            c: "var(--neon-red)",
          },
          {
            l: "Investigating",
            v: incidents.filter((i) => i.status === "investigating").length,
            c: "var(--neon-amber)",
          },
          {
            l: "Contained",
            v: incidents.filter((i) => i.status === "contained").length,
            c: "var(--neon-cyan)",
          },
          ...
          <div key={k.l} className="glass-panel rounded-lg p-4">
            ...
            <div className="mt-1 text-2xl font-mono font-semibold" style={{ color: k.c }}>
              {k.v}
            </div>
          </div>
        ))}
          ...
          <div key={i.id} className="glass-panel rounded-lg p-4 relative overflow-hidden">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2 text-[11px]">
              ...
              <div className="rounded border border-border bg-card/40 p-2">
                ...
                <div className="font-medium capitalize" style={{ color: statusColor[i.status] }}>
                  {i.status}
                </div>
              </div>
            </div>
            ...
            <div className="mt-3 flex items-center gap-3 text-[11px] text-muted-foreground">
              <span className="flex items-center gap-1">
                <User className="h-3 w-3" /> {i.assignee}
              </span>
              <span className="flex items-center gap-1">
                <Clock className="h-3 w-3" /> {ago(i.opened)}
              </span>
                ...
                return (
                  <div key={s} className="flex-1 flex items-center gap-1">
                    <div
                      ...
                      style={{
                        background: active ? statusColor[i.status] : "var(--color-secondary)",
                        boxShadow: active ? `0 0 8px ${statusColor[i.status]}` : "none",
                      }}
                    />
                  </div>
                );
