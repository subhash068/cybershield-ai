  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Panel padded={false} className="p-4 flex justify-center items-center">
          <Ring value={87} label="Attack probability · 1h" color="var(--neon-red)" />
        </Panel>
        <Panel padded={false} className="p-4 flex justify-center items-center">
          <Ring value={94} label="Model confidence" color="var(--neon-cyan)" />
        </Panel>
        <Panel padded={false} className="p-4 flex justify-center items-center">
          <Ring value={78} label="Security posture" color="var(--neon-amber)" />
        </Panel>
        <Panel padded={false} className="p-4 flex justify-center items-center">
          <Ring value={62} label="Predicted incidents · 24h" color="var(--neon-purple)" />
        </Panel>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        <Panel
          ...
          title="Neural Anomaly Stream"
          subtitle="Hidden-layer activation × time"
        >
        ...
        <Panel title="AI Recommendations" subtitle="Auto-generated playbooks">
          <ul className="space-y-2.5 text-xs">
            {aiRecommendations.map((r, i) => (
              <li
                key={i}
                className="flex items-start gap-2 rounded-md border border-border bg-card/40 p-2.5"
              >
      ...
      <Panel title="Automated Incident Summaries" subtitle="Generated by CyberShield AI">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
          {aiSummaries.map((s) => (
            <div
              key={s.title}
              className="rounded-md border border-border bg-card/40 p-4 relative overflow-hidden"
            >
              <div
                className="absolute top-0 right-0 h-16 w-16"
                style={{
                  background:
                    "radial-gradient(circle at top right, var(--neon-cyan), transparent 60%)",
                  opacity: 0.35,
                }}
              />
              ...
              <div className="mt-3 text-[10px] font-mono text-muted-foreground">
                confidence: <span className="neon-text">{s.confidence}%</span>
              </div>
            </div>
          ))}
        </div>
